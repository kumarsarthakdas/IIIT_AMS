import { CategoryOutlined, EmailOutlined } from '@mui/icons-material';
import { useGetIdentity } from '@pankod/refine-core';
import { Box, Stack, Typography } from '@pankod/refine-mui';
import { Link } from '@pankod/refine-react-router-v6'

import { DepartmentCardProp, InfoBarProps } from 'interfaces/department'
...
const DepartmentCard = ({ id, name, email, avatar, noOfAssets }: DepartmentCardProp) => {
  const { data: currentUser } = useGetIdentity();

  const generateLink = () => {
    if (currentUser.email === email) return '/my-profile'

    return `/departments/show/${id}`;
  }
    ...
    >
      ...
      <Stack direction="column" justifyContent="space-between" flex={1} gap={{ xs: 4, sm: 2 }}>
        <Stack gap={2} direction="row" flexWrap="wrap" alignItems="center">
          ...
          <Typography fontSize={14} color="#6d8278">IIIT Bhubaneswar</Typography>
        </Stack>
        <Stack direction="row" flexWrap="wrap" justifyContent="space-between" alignItems="center" gap={2}>
          <InfoBar
            icon={<EmailOutlined sx={{ color: '#6d8278' }} />}
            name={email}
          />
          <InfoBar
            icon={<CategoryOutlined sx={{ color: '#6d8278' }} />}
            name={`${noOfAssets} Assets`}
          />
        </Stack>
      </Stack>
    </Box>
